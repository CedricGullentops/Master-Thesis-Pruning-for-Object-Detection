...
log = logging.getLogger('lightnet.FLIR.test')
...
class TestEngine:
    ...
    def __call__(self):
        ...
        if self.detection is not None:
            def get_img_dim(name):
                with Image.open(f'../eavise-brugge_flir/data/VOCdevkit/{name}.jpg') as img:
                    return img.size
    ...
    def test_loss(self):
        ...
        if self.loss == 'percent':
            ...
            log.info(f'Loss:{loss_tot:.5f} (Coord:{loss_coord:.2f}% Conf:{loss_conf:.2f}%)')
        else:
            log.info(f'Loss:{loss_tot:.5f} (Coord:{loss_coord:.2f} Conf:{loss_conf:.2f})')
